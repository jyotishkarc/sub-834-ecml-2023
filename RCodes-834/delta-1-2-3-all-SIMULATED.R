...
selection <- c(1,2,3,4,5)     ## Choose which examples to run
...
for(h in selection){
   ...
   for(k in 1:length(d.seq)){
      ...
      ## Appending the means and standard errors of the trials
      res.list[[h]][[k]] <- cbind(c(error.prop.1, NA, mean(error.prop.1), 
      ...
      colnames(res.list[[h]][[k]]) <- c('del.1','del.2','del.3')
      
      print(proc.time() - start.time)
   }
   
   names(res.list[[h]]) <- c("d=5","d=10","d=25","d=50","d=100","d=250","d=500","d=1000")
   ...
   names(T.mat[[h]]) <- names(res.list[[h]])
   print(Sys.time())
}
