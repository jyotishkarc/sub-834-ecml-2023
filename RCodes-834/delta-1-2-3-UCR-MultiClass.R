########## Implementation of the proposed classifiers for datasets from the UCR database
...
## Path for the dataset 'Computers'. Make it a comment if not intended to run.
path <- "Datasets/UCR-Computers/Computers"

## Path for the dataset 'DodgerLoopDay'. Uncomment the next line if intended to run.
# path <- "Datasets/UCR-DodgerLoopDay/DodgerLoopDay"

init.train.data <- read.delim(paste0(path, "_TRAIN.tsv"), 
                              header = FALSE, 
                              stringsAsFactors = FALSE)

init.test.data <- read.delim(paste0(path, "_TEST.tsv"), 
                             header = FALSE, 
                             stringsAsFactors = FALSE)
...
for(u in 1:iterations){
   ...
   for(ind in 1:nrow(classes.mat)){
      ...
      Z <- dataset[c(test.index[[u]][[A]],
                     test.index[[u]][[B]]), -1]    ## Test Observations
